...
//	data2=malloc(100000*sizeof(uint64_t));
	...
	data2=(uint64_t *)malloc(sizeof(uint64_t)*blocksize);
	...
	do{
		...
		if (elementsRead==number_of_blocks){
			...
			{
//				printf("Nº sample[%d] AI0=%.6fV, ",sample_counter,dmaaivalues[(x*2)]*p_DrvPvt.CVADC);
//				printf("AI1=%.6fV, \n",(double)dmaaivalues[(x*2)+1]*p_DrvPvt.CVADC);
				sample_counter++;
				if (sample_counter>=atoi(argv[6]))
					break;
			}
			k++;
			if (k %100 ==0)
	...
	msgerr(st,6,irio_getDMATtoHostData irio_setDMATtoHostEnable irio_getDMATtoHostEnable,status);
//	free(dmaaivalues);
